...
{
    public class Command_modiftaux : IRocketCommand
    {
        ...
        public string Name
        {
            get { return "modiftaux"; }
        }
        public string Help
        {
            get { return "modifie le taux d'un utilisateur"; }
        }
        ...
        public void Execute(IRocketPlayer caller, params string[] command)
        {
            if (command.Length == 2)
            {
                ...
                {
                    if (float.Parse(command[1]) > 1)
                    {
                        if(BinaryBank.Instance.BinaryBankDatabase.ModifTaux(command[0], command[1]))
                        {
                    ...
                    {
                        UnturnedChat.Say(caller, "Le taux chois est improbable", Color.red);
                    }
                }
                else
                {
